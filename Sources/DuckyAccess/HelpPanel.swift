...
final class HelpPanelController: NSObject, WKNavigationDelegate {
    ...
    <h2>Command permissions</h2><p>Choose <b>Permissions → Full Access</b> in the menu bar to run routine steps without per-app prompts. <b>Ask before actions</b> confirms each shortcut plan, plus computer actions outside Calculator/TextEdit and text insertion in TextEdit. Closing tabs in a shortcut plan and sensitive actions still ask in both modes. The choice is saved and applies to the next command; it does not grant macOS permissions or change DICT and literal shortcuts.</p>
