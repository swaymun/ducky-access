...
final class HelpPanelController: NSObject, WKNavigationDelegate {
    ...
    <h2>Shortcut-first commands · experimental</h2><p>Luna first checks whether your request can use known Chrome or Codex shortcuts. Try “In Chrome, open a new tab and go to https://example.com” or “In Codex, create a new chat, then open the model picker.” The notch shows the current shortcut step. Default app bindings are used; custom shortcuts may differ.</p>
    <p>Requests that need screen context use computer control instead. Try “In Calculator, calculate twelve times seven.” Ducky Access reads the target app, performs native actions, and checks the result. No separate Computer Use helper is needed.</p>
    <p>Shortcut plans allow up to 12 actions. “Sent shortcuts” means keys were sent, not that a page finished loading. A partial failure stops without replaying the request. Say full website addresses; spoken “dot” and “slash” work.</p>
    <p><b>Click the notch, press ESC, or press CMD again to stop.</b> No further steps are dispatched; an action already sent may finish and cannot be undone. Runs stop after three minutes or 40 tool calls.</p>
    <h2>Command permissions</h2><p><b>Permissions → Full Access</b> runs your commands without Ducky approval popups, including tab-closing and sensitive actions. Requested actions can send or delete data without another Ducky confirmation.</p>
    <p><b>Ask before actions</b> confirms each shortcut plan and sensitive computer action, plus computer actions outside Calculator/TextEdit and text insertion in TextEdit. The choice is saved and applies to the next command. macOS permissions, other apps’ warnings, and click-to-stop still apply. DICT and literal shortcuts are unchanged.</p>
